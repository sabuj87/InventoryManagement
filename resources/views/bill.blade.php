  ...
  <div class="container py-3" >
    ...
    <div>
        <form class="" action="{{route('bill')}}" method="get" >
           <div class="d-flex" >
            <input class="form-control  w-25"  value="{{$billNo}}" name="billno" placeholder="Bill no"  type="number">
      ...
      <div class="row mt-3" >
           ...
           <div class="col-md-3 px-3 mt" >
            <select class="form-select" id="customer" name="custemer" >
              ...
              <option value="{{$customer->id}}"
                @if($customer->id==$inventory->customer->id)
                selected
         ...
         <div class="col-md-3 px-3 mt" >
          <input id="date" class="form-control" value="{{$inventory->date}}" placeholder="Date" type="date">
       </div>
      ...
      <div class="mt-3" >
        <table id="mainTable" class="table table-bordered table-striped">
          ...
          <tbody>
           ...
           <input id="invid" type="hidden" name="invid" value="{{$inventory->id}}" >
           <input id="billno" type="hidden" name="billno" value="{{$inventory->bill_no}}">
            @foreach($inventory->inventoryProducts as $inventoryProduct)
            <tr>
              <td>
                 {{$inventoryProduct->products->name}}

              </td>
              <td>
                
                {{ $inventoryProduct->products->rate}}
           ...
           <td>
            <p  id="{{$inventoryProduct->products->id}}totalAmount" ></p>
         ...
         <td>
          <p class="all" id="{{$inventoryProduct->products->id}}discount" ></p>
       ...
       <div class="float-end d-flex mt-2" >
        ...
        <input id="netTotal" value="{{$inventory->total_bill_amount}}" type="text">
    
      </div>
      <div class="clearfix" ></div>
      <div class="float-end d-flex mt-2 ">
        <p class="mt-3">Discount Total : </p>
        <input  id="discountTotal" value="{{$inventory->total_discount}}" type="text">
    
      </div>
      <div class="clearfix" ></div>
      <div class="float-end d-flex mt-2" >
        <p class="mt-3">Paid Amount : </p>
        <input id="paidAmount" value="{{$inventory->paid_amount}}" onkeyup="dueAm()" type="text">
    
      </div>
      <div class="clearfix" ></div>
      <div class="float-end d-flex mt-2" >
        <p class="mt-3">Due Amount : </p>
        <input id="dueAmount"  value="{{$inventory->due_amount}}"  type="text">
...
function dueAm(){
  ...
  headers: {
    ...
    $.post( "{{route('store')}}", { pname: sendPName, quantity: sendQuantity,rate:sendRate,discount:sendDiscount,invid:invido})
  .done(function( data ) {
  alert("New Inventory product added")
    ...
    function saveInvetory(){
      ...
      $.post( "{{route('storeInv')}}", { invid: invid, billno: billno,customerId:customerId,totalDiscount:totalDiscount,paidAmount:paidAmount,totalAmount:totalAmount,dueAmount:dueAmount,date:date})
  .done(function( data ) {
     alert(data);
  }).fail(function(request, status, error) {
